...
* **Фолоу-ап (следующий шаг):** перевести primary key Postgres с
  `UUID(as_uuid=False)` на нативный объект `uuid.UUID`
  (`as_uuid=True`) и отрефакторить код под это — обновить
  `UserDTO.id` / `BusinessConnectionDTO.id` / `MessageMappingDTO.id`
  на настоящий `uuid.UUID`, поправить `_to_dto`-конвертеры, фабрику
  UUIDv7 и все места, где `id` сейчас сравнивается или
  интерполируется как строка. Хранение не меняется, зато на уровне
  приложения получаем типизированный идентификатор и убираем
  последнее расхождение Python-типа между бэкендами.
