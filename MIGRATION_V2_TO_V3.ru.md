...
* Все фазы (1–6) отгружены. Миграция завершена.
* **Фолоу-ап — сделано (пост-4.1):** primary key в Postgres теперь
  нативный `UUID(as_uuid=True)`, и каждый DTO несёт
  `id: uuid.UUID | None`. SQLite-бэкенд переиспользует прежнее
  `CHAR(36)`-хранение через `UuidAsString36` TypeDecorator
  (models/base.py) — Alembic-миграция данных не требуется, — так что
  Python-слой полностью типизирован по обоим диалектам без изменений
  на диске. `uuid7()` теперь единственный общий дефолт; хендлеры
  больше не передают `id=""`.

#### 4.2 Alembic: миграции на два диалекта — **Сделано**

Оба бэкенда теперь гоняются **одной** цепочкой ревизий. Один
`alembic upgrade head` работает на любом диалекте; каждая ревизия —
no-op на «чужом» диалекте, поэтому таблица alembic-версий остаётся в
синхроне.

* `alembic/versions/0001_initial_schema.py` — защищён гардом
  `if op.get_bind().dialect.name == "postgresql": return` в начале
  `upgrade()` и `downgrade()`. Для SQLite не изменился; на Postgres —
  no-op. История v2 SQLite сохранена байт-в-байт.
* `alembic/versions/0002_postgres_parity.py` — зеркало: no-op на
  SQLite, создаёт параллельную схему на Postgres с нативными типами
  (`postgresql.UUID(as_uuid=True)`, `TIMESTAMPTZ`, нативный `BOOLEAN`)
  плюс композитный unique-констрейнт `uq_kv_store_owner_key`, от
  которого зависит `ON CONFLICT ON CONSTRAINT`-апсерт в kv-репо из
  Фазы 4.1.
* `alembic/env.py` — теперь подбирает правильный декларативный `Base`
  по диалекту активного URL
  (`from src.databases.postgres.models import Base` на Postgres, иначе
  SQLite), чтобы `compare_type=True` и autogen сверялись с нативной
  метадатой. `render_as_batch` включается только для SQLite (это
  обход отсутствующего `ALTER TABLE` в SQLite; оставлять его для
  Postgres вредно для autogen).
* `migrate-job.yaml` (k8s) не меняется — просто запускает
  `alembic upgrade head` с новым URL.

Тесты (`tests/integration/test_alembic_migrations.py`, 10
assertion-ов, 10 зелёные):

* SQLite-путь: живой upgrade/downgrade против одноразовой файловой
  БД; схема совпадает с `SqliteBase.metadata`; идемпотентный
  re-upgrade; захваченный offline-SQL показывает `VARCHAR(36)`, без
  `UUID`, без `TIMESTAMPTZ`.
* Postgres-путь: Alembic `--sql` (offline) на фейковом
  `postgresql://`-URL (живой сервер не нужен). Проверяется, что для
  каждой таблицы из metadata рендерится `CREATE TABLE`, каждый PK —
  `UUID NOT NULL`, все timestamp-ы `TIMESTAMP WITH TIME ZONE`, что
  констрейнт `uq_kv_store_owner_key` эмитится, и что обе ревизии
  оседают в `alembic_version`. Путь downgrade от `0002` проверяет, что
  все четыре таблицы дропаются.
* Опциональный live-Postgres-тест (`POSTGRES_TEST_URL=…` в окружении):
  реальный upgrade/idempotent-re-upgrade/downgrade-to-base против
  живого Postgres. По умолчанию skipped, чтобы сьют был герметичен;
  CI подключает его через Postgres service container.

Политика дальше: каждая новая миграция тестируется на **обоих**
диалектах в CI (живой SQLite + живой Postgres service; offline-SQL
diff против Postgres-диалекта — герметичная нижняя граница в
unit-тестах).

#### 4.3 Миграция данных (одноразовая, v2 SQLite → v3 Postgres) — **Сделано**
...
Оформлено как `scripts/migrate_sqlite_to_postgres.py`; запуск:
`python -m scripts.migrate_sqlite_to_postgres --source sqlite:///data/bot.db
--target postgresql+psycopg://bot:pw@host/bot`. Что делает скрипт:

* Валидирует пару URL (режет не-sqlite source и не-postgresql target).
* Прогоняет `alembic upgrade head` против цели — цепочка из Фазы 4.2
  раскатывает `users` / `business_connections` / `message_mappings` /
  `kv_store` с нативными UUID / TIMESTAMPTZ / BOOLEAN и
  констрейнтом `uq_kv_store_owner_key`.
* Preflight: отказывается запускаться, если в row-bearing таблицах
  уже есть строки. Защита от случайного повторного прогона поверх
  живых данных.
* Копирует таблицы в FK-безопасном порядке батчами через
  `INSERT … ON CONFLICT DO NOTHING`. По ходу нормализует типы: `id`
  `str`→`uuid.UUID`, `is_enabled` `int`→`bool`, наивные
  `created_at`/`updated_at` → tz-aware UTC.
* Проверяет равенство row count по каждой таблице после копии;
  поднимает ошибку при расхождении.
* Пишет JSON-отчёт в `--report` (по умолчанию
  `backups/migration-<utc-ts>.json`): per-table source/target/duration
  + пара URL с обрезанными паролями.

Чего скрипт **не делает** — всё это шаги оператора в runbook ниже:

* Заморозка записей / скейл подов.
* Снепшот источника (`sqlite3 src.db ".backup snapshot.db"`).
* Переключение `DATABASE_BACKEND=postgres` + рестарт воркеров.

Тесты (`tests/integration/test_migrate_script.py`: 2 безусловных + 4
опциональных): URL-валидация режет до любого коннекта; live-PG
happy-path копирует все строки; spot-check проверяет нормализацию
UUID/tz/bool end-to-end на цели; непустая цель отклоняется; CLI
(`main([...])`) пишет валидный JSON-отчёт с redacted-креденшелами.
Unit-хелперы (34 теста, `tests/unit/test_migrate_script.py`)
покрывают чистые функции: URL-валидаторы, `coerce_row`, `_batched`,
`verify_row_counts`, argparse, `_redact`, плюс дрейф
`TABLES_IN_ORDER` относительно `Base.metadata`.

**Cutover runbook** (шаги оператора; держим в связке с этим разделом):
...
2. **Снимок источника.**
   * `sqlite3 source.db ".backup snapshot.db"` (консистентная
     hot-копия; обёрнуто в `scripts/backup_sqlite.sh`).
   * Дальше работаем со `snapshot.db`; оригинал не трогаем до
     верификации (якорь для отката).

3. **Запустить миграционный скрипт.** Он сам делает URL-check,
   `alembic upgrade head`, preflight пустоты цели, копию и
   верификацию row count, затем пишет JSON-отчёт:
   ```
   python -m scripts.migrate_sqlite_to_postgres \
       --source sqlite:///snapshot.db \
       --target "$POSTGRES_DSN_SYNC"
   ```
   Ненулевой exit code — смотрим отчёт и перезапускаем после
   truncate цели.

4. **Переключение конфига.**
   * `DATABASE_BACKEND=postgres`, `POSTGRES_DSN=postgresql://…` в
     ConfigMap/Secret.
...
#### 4.4 План отката — **Сделано**

SQLite-PVC и Phase-0 снапшот держим **7 дней** после cutover. Три
уровня, в порядке предпочтения:

##### Уровень 1 — Config rollback (<2 мин, с потерями)

Когда: проблема проявилась в первые минуты после cutover и объём
пост-cutover записей ничтожен.

1. Переключить ConfigMap: `DATABASE_BACKEND=sqlite`, вернуть mount
   SQLite-тома (`DATABASE_PATH=/data/bot.db`).
2. `kubectl rollout restart deployment/worker-tasks
   deployment/worker-delivery statefulset/update-consumer
   deployment/webhook-receiver`.
3. Проверить, что частота ошибок хэндлеров и глубина
   `delivery_queue` устаканились.

Компромисс: записи, которые легли в Postgres после cutover, теряются.
Приемлемо только если Postgres-сторона подтверждённо сломана и
прошло буквально несколько минут трафика.

##### Уровень 2 — Data rollback через обратный скрипт (<30 мин, без потерь)

Когда: Postgres сломан и потери записей из уровня 1 неприемлемы.
Нужно короткое окно обслуживания.

1. **Заморозить запись на стороне Postgres.** Скейлим воркеров в 0:
   ```
   ...
   ```
2. **Запустить обратный копир** в свежий SQLite-файл. Скрипт
   валидирует URL, делает `alembic upgrade head` на SQLite-цели,
   preflight "пусто ли", пер-таблично копирует с `ON CONFLICT DO
   NOTHING`, верифицирует row count и пишет JSON-отчёт:
   ```
   python -m scripts.migrate_postgres_to_sqlite \
       --source "$POSTGRES_DSN_SYNC" \
       --target sqlite:///data/bot-rollback.db
   ```
   Ненулевой exit означает, что верификация не прошла; смотрим отчёт
   в `backups/rollback-<ts>.json` и перезапускаем после очистки
   цели.
3. **Переключить конфиг.** `DATABASE_BACKEND=sqlite`,
   `DATABASE_PATH=/data/bot-rollback.db`, подмонтировать SQLite-том
   (перебиндить PVC `tg-bot-data`, если он был отцеплен).
4. **Перезапустить воркеров**, отскейлить обратно. Накопленные
   апдейты дренируются.

Свойства обратного скрипта:

* Симметричен `migrate_sqlite_to_postgres.py` — тот же порядок
  таблиц, та же батчевая механика, та же схема отчёта.
* `coerce_row` превращает Postgres `uuid.UUID` в дефис-разделённые
  CHAR(36) строки — тот самый v2-формат, который `UuidAsString36`
  читает обратно без потерь.
* Булев и timestamps проходят без изменений; SQLAlchemy-шные SQLite
  `Boolean` и `DateTime` принимают нативные `bool`/`datetime`.
* Отказывается работать на непустой цели: оператор
  усекает/подставляет свежий путь и перезапускает, если предыдущий
  прогон упал посередине.
* В JSON-отчёте пароль редактируется в `***`.

##### Уровень 3 — Ядерный: восстановление Phase-0 SQLite-снапшота

Когда: уровни 1+2 невозможны (Postgres нечитаем, обратный скрипт
падает на кривых данных). Смиряемся с потерей записей.

1. Восстановить SQLite-файл из Phase-0 снапшота (`backups/
   pre-cutover-*.db`).
2. Переключить конфиг как в уровне 1.
3. Перезапустить воркеров, отскейлить обратно.

Все пост-cutover записи теряются. Фиксируем, что именно потеряно из
Postgres, для ручной сверки, если это в принципе реально.

##### После 7-дневного soak

* Удалить PVC `tg-bot-data`.
* Убрать SQLite-бэкенд из дефолтного factory-пути (код оставить
  для локальной разработки и тестов).
* Переименовать `backup-sqlite` CronJob → `backup-postgres`
  (`pg_dump -Fc`, gzip, тот же PVC + retention window).

#### 4.5 Изменения в k8s и ops — **Сделано**

* `k8s/base/postgres.yaml` (новый):
  * `PersistentVolumeClaim` `postgres-data` (5Gi RWO).
  * `StatefulSet` на одну реплику для self-hosted варианта (HA — не
    цель, одной Postgres хватит для нагрузки этого бота).
    `postgres:16-alpine`, PGDATA на PVC, readiness + liveness probes
    через `pg_isready`. Requests 100m/256Mi, limits 500m/1Gi.
  * `Service` (ClusterIP) на `5432`.
  * Все учётки из `tg-bot-secrets`: `POSTGRES_USER`,
    `POSTGRES_PASSWORD`, `POSTGRES_DB`.
* `k8s/base/kustomization.yaml`: подключает `postgres.yaml`.
* `k8s/base/configmap.yaml`: добавляет `POSTGRES_HOST=postgres`,
  `POSTGRES_PORT=5432` (совпадает с внутрикластерным Service).
* `k8s/base/secret.yaml.example`: добавляет плейсхолдеры
  `POSTGRES_USER`, `POSTGRES_PASSWORD`, `POSTGRES_DB`.
* `k8s/base/backup-cronjobs.yaml`: добавляет CronJob
  `backup-postgres` (`pg_dump -Fc`, ежедневно 02:45 UTC, retention
  30 дней, `suspend: true` до cutover). Использует
  `POSTGRES_HOST`/`POSTGRES_PORT` из ConfigMap — работает и для
  self-hosted, и для managed Postgres.
* `k8s/overlays/managed-postgres/` (новый): убирает внутрикластерный
  StatefulSet + Service + PVC, патчит ConfigMap на
  `DATABASE_BACKEND=postgres` с managed-эндпоинтом в
  `POSTGRES_HOST`, включает `backup-postgres`, отключает
  `backup-sqlite`.
* `k8s/base/worker-tasks.yaml`: убран mount PVC `tg-bot-data` —
  воркеры обращаются к БД по сети через Postgres.
* `k8s/overlays/scaled/update-consumer.yaml`: убран SQLite PVC mount
  и связанный комментарий.
* `k8s/base/migrate-job.yaml`: упрощён до env-only (без PVC);
  `alembic upgrade head` работает для обоих бэкендов через
  dual-dialect цепочку.
...
### Фаза 5 — Автоскейлинг и observability — **Сделано**

Цель: сделать систему саморегулируемой и прозрачной.

**Метрики Prometheus** — все отгружены, HTTP `/metrics` подключён:

* `src/tasks/metrics.py` расширен тремя инструментами:
  * `receiver_requests_total{outcome}` — счётчик: `published`,
    `dedup`, `publish_error`.
  * `receiver_publish_duration_seconds` — гистограмма с суб-мс
    корзинами (0.001–1.0s) для латенси публикации в RabbitMQ.
  * `handler_duration_seconds{shard}` — гистограмма (0.05–30s) для
    времени выполнения Celery-задачи `handle_update`, промаркирована
    именем шард-очереди (из `delivery_info.routing_key`).
* `src/receiver/app.py`: эндпоинт `GET /metrics` отдаёт дефолтный
  реестр `prometheus_client` через `generate_latest()`.
* `src/tasks/processing.py`: `handle_update` инструментирован
  `handler_duration_seconds`.

**HPA / KEDA ScaledObjects** — `k8s/overlays/scaled/`:

* `webhook-receiver-hpa.yaml` (существующий, по CPU, 2–10 реплик).
* `keda-scaled-objects.yaml` (новый):
  * `TriggerAuthentication` `rabbitmq-trigger-auth` — подключён к
    `RABBITMQ_HTTP_URL` из ConfigMap.
  * `ScaledObject` `worker-delivery` — глубина `delivery_queue`
    ≥ 50, min 1, max 3 (верхняя граница определяется Redis
    token-bucket, не количеством подов).
  * `ScaledObject` `worker-tasks` — глубина `tasks_queue`
    ≥ 20, min 1, max 8.
* `update-consumer` StatefulSet НЕ автоскейлится: replica count =
  `UPDATES_SHARDS` (1:1 под-на-шард для порядка). Скейлить его —
  ре-шардинг (см. §5, runbook ре-шардинга).

**Prometheus Operator ресурсы** — `prometheus-monitors.yaml` (новый):

* `ServiceMonitor` `webhook-receiver` — скрейпит `/metrics` каждые 15s.
* `PodMonitor` `celery-workers` — шаблон для случая, когда
  Celery-воркеры начнут отдавать метрики на порту (например, через
  `start_http_server(9090)` в `worker_init`-сигнале).

**Grafana dashboard** — `k8s/base/grafana-dashboard.json` (новый):

Шесть панелей: delivery rates, retries + DLQ, dedup hit/miss, receiver
requests по outcome, receiver publish latency (p50/p90/p99), handler
latency по шардам (p50/p99).

**ConfigMap** — добавлен `RABBITMQ_HTTP_URL` для KEDA RabbitMQ scaler.

**Тесты** — `tests/unit/test_metrics.py` (6 тестов): типы метрик,
имена лейблов, кастомные корзины гистограмм.
`tests/integration/test_receiver_metrics.py` (6 тестов): формат
`/metrics`, инкременты `receiver_requests_total` для
published/dedup/publish_error, `receiver_publish_duration_seconds`
наблюдает при публикации, `handler_duration_seconds` декларация.

Про порядок (без изменений): если `worker-tasks` масштабируется выше
N (количества шардов), лишние реплики не могут потреблять
упорядоченные шарды без нарушения правила «один потребитель». Два
варианта:
...
### Фаза 6 — Cutover и уборка  ✅ Shipped

* `setWebhook` → публичный URL receiver'а.
* Удалён overlay `k8s/overlays/webhook/` (4 файла).
* Удалена `_run_webhook()` из `src/bot/runner.py`; `"webhook"` убран из
  Literal `MODE` в settings — остались только `polling` | `receiver`.
* Polling overlay оставлен для локальной разработки.
* `README.md` обновлён — снят баннер «v3 status: in progress», секция
  про масштабирование влита в основное описание архитектуры, удалены
  все упоминания webhook-режима.
...
- [x] Фаза 4.2: dual-dialect Alembic-миграция (`0002_postgres_parity`) + dialect-aware `env.py` + offline-SQL + опциональный live-Postgres тест
- [x] Фаза 4.3: `scripts/migrate_sqlite_to_postgres.py` + unit-тесты (coerce_row, валидаторы, argparse, redact) + опциональные live-Postgres integration-тесты (копия фикстуры, отказ на непустой цели, JSON-отчёт через CLI)
- [x] Фаза 4.4: обратный скрипт + задокументированный runbook отката
- [x] Фаза 4.5: `k8s/base/postgres.yaml` + overlay `managed-postgres` + CronJob `backup-postgres`; снятие mount PVC `tg-bot-data` с воркеров
- [x] Фаза 5: HPA + KEDA + метрики Prometheus + dashboard
- [x] Фаза 6: cutover, уборка, консолидация README
