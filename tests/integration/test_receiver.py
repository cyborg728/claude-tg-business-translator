"""End-to-end tests for the webhook-receiver FastAPI app.

The publisher is stubbed — we're testing the receiver's *glue* (secret
check, JSON parsing, dedup claim, publish dispatch, HTTP responses),
not RabbitMQ itself. Redis is the real fakeredis instance from
``conftest.py`` so dedup exercises the same code path as production.
"""
...
class _StubPublisher:
    """In-memory stand-in for :class:`UpdatePublisher`.

    Records every publish call so tests can assert on the sequence and
    lets individual tests flip ``connected`` / ``raise_on_publish`` to
...
async def test_empty_configured_secret_disables_check(fake_redis):
    # Operators may leave WEBHOOK_SECRET_TOKEN empty in dev; settings.py
    # warns but doesn't reject. The receiver must mirror that behaviour.
    publisher = _StubPublisher()
...
async def test_invalid_json_is_400(receiver):
    client, publisher, settings = receiver
    resp = client.post(
        settings.webhook_path,
        content=b"not-json",
...
async def test_duplicate_update_id_is_200_and_does_not_publish(receiver):
    ...
    # Second POST must NOT reach the publisher.
    assert len(publisher.calls) == 1
...
async def test_update_without_chat_publishes_with_none(fake_redis):
    # business_connection (or similar) has no chat; the receiver should
    # still forward the update, just with chat_id=None.
    publisher = _StubPublisher()
    settings = _settings()
    app = create_app(settings, fake_redis, publisher)  # type: ignore[arg-type]
...
async def test_readyz_503_when_redis_fails(fake_redis):
    # Swap in a redis client whose ping() raises to simulate outage.
    publisher = _StubPublisher()
    settings = _settings()
