"""Tests for the /metrics endpoint and receiver-level Prometheus counters.

Uses the same stub publisher and fake_redis as test_receiver.py.
"""
...
def _counter_value(name: str, labels: dict[str, str]) -> float:
    """Read a counter's current value from the default registry."""
...
async def test_dedup_request_increments_counter(metrics_receiver):
    ...
    # Second request with same update_id → dedup.
    client.post(settings.webhook_path, json=_update(update_id=9002), headers=headers)
...
# ── handler_duration_seconds declared correctly ───────────────────────────


def test_handler_duration_metric_exists():
    from src.tasks.metrics import handler_duration_seconds

    assert handler_duration_seconds._name == "handler_duration_seconds"
